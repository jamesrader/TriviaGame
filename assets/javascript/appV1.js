$(document).ready(function() {
  ...
  var phrases = [
    "You weren't really trying, were you?",
    "Atrocious!",
    "Abysmal!",
    "Pathetic!",
    "Awful!",
    ...
    "Well done!",
    "Great job!",
    "Fantastic! You must be a genius!"
  ];
  var answersText = "";
  var q = -1;
  var numQuestions = 10;
  ...
  function resetGame() {
    ...
    //Reset and start the music
    $("#music-audio")[0].stop;
    $("#music-audio")[0].pause;
    $("#music-audio").prop("currentTime", 0);
  ...
  function quiz() {
    if (q < questionsArray.length - 1) {
      ...
      question = $("<textarea/>")
        .html(questionsArray[q].question)
        .val();
      ...
      for (j = 0; j < answersNum; j++) {
        ...
        if (answersOrder[j] == 0) {
          answersText =
            "<button type='button' class='btn btn-link btn-lg' id=" +
            answersOrder[j] +
            ">" +
            letters[j] +
            ". " +
            questionsArray[q].correct_answer +
            "</button>";
          ...
          answersText =
            ...
            letters[j] +
            ". " +
            questionsArray[q].incorrect_answers[answersOrder[j] - 1] +
            "</button>";
          $("#answer" + k).html(answersText);
        }
      }
      ...
      $(".btn").on("click", function(event) {
        ...
        if (chosenAnswer == 0) {
          ...
          $("#messageArea").html(
            "<span class='wrong'>Sorry, that's wrong.</span>  The correct answer was:  " +
              letters[answersOrder.indexOf(0)] +
              ". " +
              questionsArray[q].correct_answer
          );
          wrong++;
      ...
      speechString =
        "Quiz complete. Your score was " + correctPercent + " percent.";
      ...
      responsiveVoice.speak(
        phrases[Math.round(correctPercent / 10)],
        "UK English Female",
        { volume: 1 },
        { rate: 0.5 }
      );
  ...
  function countdown() {
    ...
    if (timer === 0) {
      ...
      $(".btn").prop("disabled", true);
      $("#messageArea").html(
        "<span class='wrong'>You ran out of time!</span>  The correct answer was:  " +
          letters[answersOrder.indexOf(0)] +
          ". " +
          questionsArray[q].correct_answer
      );
